...
function showSomeoneRepos(json) {
    if (json != null) {
        var contentURL = parent.document.getElementById('content').contentWindow.location.href;
        if(!contentURL.endsWith("content.html")){
            alert('Please go to repositories window to see repositories!');
            return;
        }
        innerContent = parent.window.frames['content'].document;
